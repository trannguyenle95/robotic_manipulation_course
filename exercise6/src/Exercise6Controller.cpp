...
void Exercise6Controller::update(const ros::Time &time, const ros::Duration &period) {
    ...
    tau = J.transpose() * (kp * P * Xe + (Eigen::Matrix<double, 6, 6>::Identity() - P) * Fd);
...
Eigen::Matrix<double, 3, 1> Exercise6Controller::getPositionError(const Eigen::Isometry3d &current_pose,
    ...
    if (robot_name == "lumi1") {
        // use robot_name to distinguish between robots
    }
    ...
    const auto time = time_from_start;
    const auto delta_time = 10;
    Eigen::Translation3d guide_point(0.0, 0.2, 0.0);
    if (time < 1 * delta_time) {
        guide_point = Eigen::Translation3d(0.0, 0.0, 0.0);
    } else if (time < 2 * delta_time) {
        const auto dtime = (time - 1 * delta_time) / delta_time;
        guide_point = Eigen::Translation3d(0.0, 0.0, 0.2 * dtime);
    } else if (time < 3 * delta_time) {
        const auto dtime = (time - 2 * delta_time) / delta_time;
        guide_point = Eigen::Translation3d(0.0, 0.2 * dtime, 0.2);
    } else if (time < 4 * delta_time) {
        const auto dtime = (time - 3 * delta_time) / delta_time;
        guide_point = Eigen::Translation3d(0.0, 0.2, 0.2 - 0.2 * dtime);
    }
    if (robot_name == "lumi2") {
        guide_point.translation()[1] *= -1; //robots rotated by rotz(pi)
    }

//    std::cout << guide_point.translation().transpose() << std::endl;
    const Eigen::Affine3d  desired_pose = guide_point * starting_pose;
