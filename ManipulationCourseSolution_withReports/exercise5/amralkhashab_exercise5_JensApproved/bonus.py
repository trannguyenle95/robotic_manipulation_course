...
ofc1, ofc2, onorm = 0, 0 ,0
ofc3, ofc4, onorm2 = 0, 0 ,0
...
for i in range(100000):
    ...
    if(check and check2):
        ...
        d3 = from_edge(lines,line,firstpoint)
        d4 = from_edge(lines, line, secondpoint)
        ...
        if d < min:
            ...
            opoint = firstpoint
            opoint2 = secondpoint
            ofc1, ofc2, onorm = fc1, fc2, norm
            ofc3, ofc4, onorm2 = fc3, fc4, norm2
            print(d1, d2)
...
###################################draw points#################################
draw_fc(lines, optimal_line, opoint)
draw_fc(lines, optimal_line2, opoint2)
plt.scatter(opoint[0], opoint[1], s=40)
plt.scatter(opoint2[0], opoint2[1], s=40)
plt.plot([opoint[0], opoint2[0]], [opoint[1], opoint2[1]], '--k')

#write file
angle1 = onorm.alpha(ox)
angle2 = onorm2.alpha(ox)
dataFile = open('data/grasp_data.txt', "w")
if opoint[0]>0:
    dataFile.write(str(opoint[0]*0.1) + ' ' + str(opoint[1]*0.1)  + " " + str(angle1) + "\n")
    dataFile.write(str(opoint2[0]*0.1) + ' ' + str(opoint2[1]*0.1) + " " + str(angle2) + '\n')
else:
    dataFile.write(str(opoint2[0] * 0.1) + ' ' + str(opoint2[1] * 0.1) + " " + str(angle1) + '\n')
    dataFile.write(str(opoint[0] * 0.1) + ' ' + str(opoint[1] * 0.1) + " " + str(angle2) + "\n")
