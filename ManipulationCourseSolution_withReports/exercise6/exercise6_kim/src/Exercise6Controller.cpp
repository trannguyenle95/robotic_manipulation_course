...
Eigen::Matrix<double, 3, 1> Exercise6Controller::getPositionError(const Eigen::Isometry3d &current_pose,
    ...
    float target[3] = {0.0, 0.0, 0.1};
    if (time_from_start >= 10.0 && time_from_start < 20.0) {
        target[2] += 0.02 * time_from_start - 0.2;
    }
    else if (time_from_start >= 20.0 && time_from_start < 30.0) {
        target[2] += 0.2;
        if (robot_name == "lumi1") target[1] = 0.02 * time_from_start - 0.4;
        else target[1] = -0.02 * time_from_start + 0.4;
    }
    else if (time_from_start >= 30.0 && time_from_start < 40.0) {
        target[2] += -0.02 * time_from_start + 0.8;
        if (robot_name == "lumi1") target[1] = 0.2;
        else target[1] = -0.2;
    }
    else if (time_from_start >= 40.0) {
        if (robot_name == "lumi1") target[1] = 0.2;
        else target[1] = -0.2;
    }
    ...
    //const Eigen::Isometry3d desired_pose = starting_pose * Eigen::Translation3d(target[2], target[1], target[0]);
    const Eigen::Isometry3d desired_pose = Eigen::Isometry3d::Identity() * Eigen::Translation3d(target[0], target[1], target[2]);
